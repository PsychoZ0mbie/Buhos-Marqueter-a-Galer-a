...
tableArticulo = $('#tableProductos').dataTable( {
    "aProcessing":true,
    "aServerSide":true,
    "language": {
...
tablePapelera = $('#tablePaper').dataTable( {
    ...
    "ajax":{
        "url": " "+base_url+"/Productos/getPapelera",
        "dataSrc":""
    },
    "columns":[
        {"data":"title"},
...
window.addEventListener('load',function(){
    ...
    let formArticulo = document.querySelector("#formProducto");
...
/******************************************************ATTRIBUTES***************************************************************/
tableAttribute = $('#tableAtributo').dataTable( {
    "aProcessing":true,
    "aServerSide":true,
    "language": {
        "url": "//cdn.datatables.net/plug-ins/1.10.20/i18n/Spanish.json"
    ...
    "ajax":{
        "url": " "+base_url+"/Productos/getAttributes",
        "dataSrc":""
    },
    "columns":[
        {"data":"name"},
...
if(document.querySelector("#btnActionForm")){
    let btnAttribute = document.querySelector("#btnActionForm");
    btnAttribute.onclick=function(e){
        let formAttribute = document.querySelector("#formAttribute")
        let name = document.querySelector("#txtName").value;

        if(name ==""){
            swal("Atención","Por favor, escribe el atributo.","error");
            return false;
        }

        let request =(window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject("Microsoft.XMLHTTP");
        let ajaxUrl = base_url+"/Productos/setAttribute";

        let formData = new FormData(formAttribute);
        request.open("POST",ajaxUrl,true);
        request.send(formData);
        ...
        request.onreadystatechange=function(){
            if(request.status ==200 && request.readyState == 4){
                let objData = JSON.parse(request.responseText);
                if(objData.status){
                    tableAttribute.api().ajax.reload();
                    formAttribute.reset();
                    swal("Atributos",objData.msg,"success");
                }else{
                    swal("Atributos",objData.msg,"error");
                }
            }
        }
    }
...
function fntDelAtt(id){
    swal({
        ...
        if(isConfirm){
            let request = (window.XMLHttpRequest) ? new XMLHttpRequest : new ActiveXObject("Microsoft.XMLHTTP");
            let ajaxUrl = base_url+"/Productos/delAttribute/"+id;
            request.open("GET",ajaxUrl,true);
            request.send();
            divLoading.style.display = "flex";
            request.onreadystatechange= function(){
